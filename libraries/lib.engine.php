...
class Engine{
	...
	public function sendNotification($userid,$message,$type){
		...
		$notifycode = $this->generateCode('webmin_notifications','NFY','NOTE_CODE');
		$stmt = $sql->Execute($sql->Prepare("INSERT INTO webmin_notifications (NOTE_CODE,NOTE_USERID,NOTE_MESSAGE,NOTE_TYPE,NOTE_DATEADDED) VALUES(?,?,?,?,?) "),[$notifycode,$userid,$message,$type,$notifydate]);
		print $sql->ErrorMsg();
		if($stmt==true){
