        ...
        if($stmt->RecordCount() > 0){
            ...
            $checkinit = $sql->Execute($sql->Prepare("SELECT INIT_ID FROM app_init WHERE INIT_USRCODE=".$sql->Param('a')." "),array($this->userid));
                if($checkinit->RecordCount() >0){
                    ...
                    $code = $id['INIT_ID'];
                    $sql->Execute($sql->Prepare("UPDATE app_init SET INIT_APIKEY=".$sql->Param('a').",INIT_USRCODE=".$sql->Param('b').",INIT_STATUS='2', INIT_CIPHER = ? WHERE INIT_ID=".$sql->Param('c').""),array($userapikey,$clientid,$cipher,$code));
                    $this->response(array('msg'=>'success','data'=>$result,'cipher'=>$cipher,'imgurl'=>$this->api_imgurl),200);
                }else{
